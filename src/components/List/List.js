...
export default function List ({ starships, changeStarship, changeId }) {
  ...
  const handleSelectStarship = (name) => {
    const filteredItem = filterStarships(name, starships)
        ...
        starships && (
          starships.map((starship, index) => {
            return <li key={index} className="list__item" data-testid={`element-${index}`}>
              <Link onClick={(e) => handleSelectStarship(e.target.firstChild.textContent)} to={`/starships/${starship.name}`}>
                <h3>{starship.name}</h3>
                <p>{starship.model}</p>
              </Link>
            </li>
